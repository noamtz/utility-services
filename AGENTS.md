...
- The TypeScript workspace, SST application, React dashboard, shared contracts, REST/observability core, invite-only owner/project control, owner API-key lifecycle, and reusable project-authentication boundary are implemented. Identity/control lives under `packages/backend/src/modules/identity-control`; project authentication lives under `packages/backend/src/modules/project-authentication`; later File Management and usage/pricing slices remain unimplemented.
