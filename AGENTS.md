...
- The TypeScript workspace, SST application, React dashboard, shared contracts, REST/observability core, invite-only owner/project control, owner API-key lifecycle, reusable project-authentication boundary, usage/pricing bounded context, and File Management direct-upload/list/metadata/completion slice are implemented. Identity/control lives under `packages/backend/src/modules/identity-control`; project authentication lives under `packages/backend/src/modules/project-authentication`; usage/pricing lives under `packages/backend/src/modules/usage-pricing`; File Management lives under `packages/backend/src/modules/file-management`. RUS-06+ download, lifecycle, metering, dashboard, and release-hardening slices remain unimplemented.
