...
- The TypeScript workspace, SST application, React dashboard, shared contracts, REST/observability core, invite-only owner/project control, owner API-key lifecycle, reusable project-authentication boundary, and usage/pricing bounded context are implemented. Identity/control lives under `packages/backend/src/modules/identity-control`; project authentication lives under `packages/backend/src/modules/project-authentication`; usage/pricing lives under `packages/backend/src/modules/usage-pricing`; later File Management and ingestion/presentation slices remain unimplemented.
