...
In [Part 1](./codebase-search-and-lsp.md), I showed you how to set up tooling. Now let's talk about something more controversial: why your architecture might be working against you.
...
Why it matters for AI: Remember why explicit dependencies matter? (See [Article 1](./codebase-search-and-lsp.md) on architectural boundaries.) FastAPI's `Depends()` makes this perfect—the agent can trace exactly what each function needs.
