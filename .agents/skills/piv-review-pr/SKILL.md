...
- **The implementation report** (if `piv-implement` wrote one — `.agents/reports/*{branch}*`) + its plan: read the
...
Write the report to `.agents/code-reviews/pr-{N}-review.md` (summary · issues by severity with `file:line` + fix ·
validation table · what's good · recommendation). Then post it:

```bash
# approve
gh pr review {N} --approve --body-file .agents/code-reviews/pr-{N}-review.md
# request changes
gh pr review {N} --request-changes --body-file .agents/code-reviews/pr-{N}-review.md
# or just comment (draft PRs / advisory)
gh pr comment {N} --body-file .agents/code-reviews/pr-{N}-review.md
```
