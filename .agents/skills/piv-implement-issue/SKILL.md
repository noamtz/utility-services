...
- Confirm the RCA exists at `docs/issues/issue-<issue-number>.md`.
- GitHub CLI installed and authenticated (optional, for status updates)

## RCA Document to Reference

Read RCA: `docs/issues/issue-<issue-number>.md`
...
```bash
gh issue comment <issue-number> --body "Fix implemented in commit [commit-hash]. Ready for review."
```

**Update issue labels (if needed):**
```bash
gh issue edit <issue-number> --add-label "fixed" --remove-label "bug"
```

**Close the issue (if not using auto-close via commit message):**
```bash
gh issue close <issue-number> --comment "Fixed and merged."
```
