...
`the user's request` is the list of branches to create.
...
Spawn all setup subagents at once. Give every subagent the same bounded prompt, substituting only `BRANCH`
and `PORT`:

```
Set up a git worktree for branch: BRANCH   (assigned port: PORT, only relevant if you start a service)

Follow the checklist in .agents/skills/worktree-create/references/worktree-setup.md. Concretely:
